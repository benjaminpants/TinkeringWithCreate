...
import static com.simibubi.create.content.equipment.armor.BacktankItem.SLOT;

public class TinkersBacktankArmorLayer<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T, M> {

    public TinkersBacktankArmorLayer(RenderLayerParent<T, M> renderer) {
        super(renderer);
    }
    ...
    public void render(PoseStack ms, MultiBufferSource buffer, int light, LivingEntity entity, float yaw, float pitch,
        ...
        BacktankItem baseItem = BacktankItem.getWornBy(entity);
        if (baseItem == null)
            return;

        if (!(baseItem instanceof TinkersBacktankItem item))
            return;
        ...
        BakedModel blockModel = dispatcher.getBlockModel(renderedState);
        ModelData data = item.getBlockModelData(entity.getItemBySlot(SLOT));
        ...
        renderer.renderModel(ms.last(), vc, renderedState, blockModel, 1f, 1f, 1f, light, OverlayTexture.NO_OVERLAY, data, RenderType.cutout());
    ...
    public static void registerOn(EntityRenderer<?> entityRenderer) {
        ...
        if (!(livingRenderer.getModel() instanceof HumanoidModel))
            return;
        TinkersBacktankArmorLayer<?, ?> layer = new TinkersBacktankArmorLayer<>(livingRenderer);
        livingRenderer.addLayer((TinkersBacktankArmorLayer) layer);
    }
